...
C<GDOME NO_MODIFICATION_ALLOWED_ERR>: Raised if this map is readonly.
...
C<GDOME INUSE_ATTRIBUTE_ERR>: Raised if arg is an Attr that is already an attribute of another Element object. The DOM user must explicitly clone Attr nodes to re-use them in other elements.
